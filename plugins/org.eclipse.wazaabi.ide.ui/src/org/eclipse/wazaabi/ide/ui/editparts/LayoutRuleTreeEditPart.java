...
public class LayoutRuleTreeEditPart extends AbstractTreeEditPart {
	...
	protected void refreshVisuals() {
		...
		Image image = Activator.getDefault().getImageRegistry()
				.get(getLayoutRuleModel().eClass().getName());
		TreeItem item = (TreeItem) getWidget();
		if (image != null)
			image.setBackground(item.getParent().getBackground());
		setWidgetImage(image);
		setWidgetText(getLayoutRuleModel().eClass().getName());
	}

	public LayoutRule getLayoutRuleModel() {
		return (LayoutRule) getModel();
	}
