...
public abstract class AbstractWidgetEditPart extends AbstractEditPart implements
	...
	public boolean styleRuleRemoved(StyleRule oldRule) {
		...
		if (rule == null) {
			...
			if (defaultStyleRule != null) {
				...
				StyleRule removedRule = createBlankStyleRule(propertyName);
				if (getWidgetView().needReCreateWidgetView(removedRule))
					return true;
	...
	public void processPostUIBuilding() {
		...
		List<Object> services = getRegistry().getServices(
				AnnotationManagerFactory.class);
		for (Object service : services)
			if (service instanceof AnnotationManagerFactory)
				for (Annotation annotation : ((Widget) getModel())
						.getAnnotations()) {
					AnnotationManager annotationManager = ((AnnotationManagerFactory) service)
