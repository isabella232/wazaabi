...
//		System.out.println("adding " + editPartFactory);
		editPartFactories.add(editPartFactory);
	}

	public void removeEditPartFactory(
			EditPartFactory editPartFactory) {
//		System.out.println("removing " + editPartFactory);
		editPartFactories.remove(editPartFactory);
	}

	public EditPart createEditPart(EditPart context, Object model) {
		for (EditPartFactory editPartFactory : editPartFactories) {
			EditPart editPart = editPartFactory.createEditPart(context,
					model);
			if (editPart != null)
				return editPart;
		}
