...
public class TabbedPropertyList extends Composite {
	...
	private final TabbedPropertySheetPage propertySheetPage;
		...
		public ListElement(Composite parent, final ITabItem tab, int index) {
			...
			addMouseListener(new MouseAdapter() {
				...
				public void mouseUp(MouseEvent e) {
					if (!selected) {
						int index = getIndex(ListElement.this);
						select(index);
						propertySheetPage.selectTab(index);
					}
				}
			});
	...
	public class TopNavigationElement extends Canvas {
		...
		private void paint(PaintEvent e) {
			...
			if (elements.length != 0) {
				...
				int textMiddle = (bounds.height - height) / 2;
				e.gc.setForeground(widgetForeground);
				e.gc.drawText("properties_not_available", textIndent,
						textMiddle);
			}
	...
	protected int getIndex(ListElement element) {
	...
	private void initColours() {
		 ...
		 * + INFO_BACKGROUND 40%
		 */
		defaultGradientStart = getColors().createColor(
	...
	private static FormColors formColors = null;
