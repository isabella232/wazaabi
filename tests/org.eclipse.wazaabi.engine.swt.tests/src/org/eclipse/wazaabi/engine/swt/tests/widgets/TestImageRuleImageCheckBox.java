...
public class TestImageRuleImageCheckBox extends AbstractTestImageRuleImageButton{
	...
	public void testModelSetImageBeforeViewerSetContentsEqualsSWTImage(){
		...
		Assert.assertFalse(swtImage.isDisposed());
	...
	@Test
	public void testModelMoveThreeImageAfterViewerSetContentsEqualsSWTImage(){
		Button swtButton = (Button) ModelMoveThreeRule(false, widget, imageRuleImage, imageRuleImage2, imageRuleImage3, "image");
		Image swtImage = swtButton.getImage();
		Assert.assertNotNull(swtImage);
		Assert.assertFalse(swtImage.isDisposed());
		Assert.assertEquals(image1.getBounds(), swtImage.getBounds());
